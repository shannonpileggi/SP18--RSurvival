---
title: Introducing parmsurvfit Package - Simple Parametric Models for Survival Analysis in R
author:
  - name: Victor Wilson
...
Observations in survival analysis may also be subject to censoring, which occurs when
...
right-censoring, which is the most common form of censoring. Right censoring can also occur if a
...
time $t$.  The hazard function computes the conditional risk that the 
...
we developed utilize parametric methods for survival analysis, which assumes that the
...
parametric models for survival data and corresponding summary statistics was not easily attained. 
...
MLE's: a plot for males (\code{level = 1}), females (\code{level = 2}), and overall. In these plots, all time to event data are plotted regardless of censoring status.
...
lists each function and its relationship to the other functions. 
...
tail probability; this can be reversed to a lower tail probability using the 
